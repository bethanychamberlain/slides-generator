...
1. **Container Apps Environment** — the "sandbox" where your app runs
2. **Container App** — the actual running instance of Slide Guide Generator
3. **Storage Account + File Share** — persistent storage for cache and usage logs
4. **Log Analytics Workspace** — for viewing container logs in Azure Portal

**Estimated cost:** ~$30-50/month for light usage (10-20 faculty). Scales automatically.
...
A resource group is like a folder for all the Azure resources this app needs.
...
```bash
# Rebuild and push the container image, then:
