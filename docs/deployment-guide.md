...
Streamlit app that converts PDF lecture slides into AI-generated study guides. Faculty sign in via Entra ID, upload slides, and export as Word, PDF, HTML, or Canvas quizzes.
...
```
  ...
  │                                                         │
  │  Streamlit App ──► Entra ID (Easy Auth or MSAL) ──► SSO │
  │       │                                                 │
  │       ▼                                                 │
...
### Optional: Easy Auth (Recommended for Azure)

On Container Apps or App Service, you can enable **Easy Auth** instead of the MSAL app registration. Azure handles SSO at the platform level — already-signed-in users get through automatically, no redirect needed.

The app detects Easy Auth headers automatically. To enable:

1. In the Azure Portal, go to your Container App / App Service → **Authentication**
2. Click **Add identity provider** → **Microsoft**
3. Choose your Entra ID tenant
4. Set **Unauthenticated requests** to **Require authentication**

With Easy Auth enabled, Step 1 (app registration) becomes optional — Azure creates and manages the registration for you.
...
2. Sign in with Microsoft (automatic with Easy Auth, or click the sign-in button with MSAL)
3. Confirm your name appears in the sidebar
...
Large PDFs (50+ pages) are CPU and memory intensive. Container limits are set to 2 GB / 2 cores in `docker-compose.yml`.
...
Monthly JSON Lines files in `/data/usage_logs/`:
